...
class BaseData:
    ...
    """
    Base class for all data processing. Functions belong here if they can be used across the board in any workflow or
    with any dataset.
    """
    ...
    def combine_partials(cls, directory, dtypes):
        ...
        Allows for saving paid API call or scrape URL calls in small chunks so as not to lose data in the case of
    ...
    def set_zip_main(cls, raw_zip):
        """
        Fixes zip codes that the raw data includes as 9-digit by separating out and returning the first 5
        """
        zip_str = str(raw_zip)
        if len(zip_str) > 5:
            return zip_str[0:5]
