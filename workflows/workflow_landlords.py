...
class BaseLandlordWorkflow(BaseData):
    ...
    """
    Base class for landlord data_root processing workflow. Contains class variables with file path names for required
...
class WkflValidateAddresses(BaseLandlordWorkflow, AddressValidator):
    ...
    INPUTS:
        ...
        - Validated corp/llc addresses (from corp/llc data_root)
        - Unvalidated corp/llc addresses (from corp/llc data_root)
...
class WkflSubsetRentals(BaseLandlordWorkflow, BaseLandlordData):
    ...
    """
    Subsets property data_root outputted by WkflPrepareProps for only rental properties.
...
class WkflPrepareCorpLLC(BaseLandlordWorkflow, CleanCorpLLC, StringMatching):
    ...
    """
    Cleans corporate and LLC data_root and merges with validated rental subsetted properties. Filters to exclude inactive
    ...
    INPUTS:
        - Corporate data_root master file
        - LLC data_root master file
        - Rental-subsetted property datasetn outputted by WkflCleanTaxRecords
        - Master names dataset
        - Common names dataset

    OUTPUTS:
        - Rental properties subset with corporate & LLC data_root merged
...
class WkflValidateCorpLLC(BaseLandlordWorkflow, CleanCorpLLC):
    ...
    """
    Merges validated address data_root to corporate and LLC subsets outputted from WkflPrepareCorpLLC. Executes one merge
    per address column (three merges for LLC data_root, two for corporate data_root). Once these merges are complete, the
    ...
    OUTPUTS:
        - Corporate dataset subsetted for rental properties with validated address data_root
        - LLC dataset subsetted for rental properties with validated address data_root
        - Rental properties dataset containing validated address data_root for associated corporations/LLCs
    """
